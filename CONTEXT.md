...
A one-shot foreground interaction that opens a temporary Window, waits for user input or closure, prints the result, and exits. A Prompt is separate from daemon-owned persistent Windows by default. Window closure without a page-submitted value is a successful Prompt result with a discriminated closure value, not a technical error. An explicit cancellation control inside Prompt content can submit an explicit cancellation value before the Prompt closes. The canonical explicit Prompt cancellation payload is `{ "type": "prompt.canceled" }`; this discriminant is reserved by the CLI Contract for explicit user cancellation in Prompt content. Prompt content should use this payload for explicit cancel controls instead of directly closing the Window. The canonical Prompt window-closure payload is `{ "type": "window.closed" }`, used when the Prompt Window closes without an app-level submitted value. A Prompt can have an explicit timeout; timeout is a technical failure rather than a cancellation result. A Prompt resolves and closes after the first page-to-CLI message, returning the raw submitted value in the command result's `result` field. Explicit Prompt terminal outcomes such as `{ "type": "prompt.canceled" }` and `{ "type": "window.closed" }` are also returned through the same `result` field. URL-based Prompts require the page bridge; remote URL Prompts require both remote loading and bridge allowance.

## Prompt Result

The JSON value placed in the `result` field by `glimpse prompt`. Prompt Results include raw page-submitted values, the reserved explicit cancellation value `{ "type": "prompt.canceled" }`, and the synthesized window-closure value `{ "type": "window.closed" }`. Prompt content should not submit raw `null`; in v1, a raw `null` Prompt Result is treated as Window closure. Prompt timeout is not a Prompt Result because it is a technical failure.
