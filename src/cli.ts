...
addOpts(addHtml(program.command('prompt'))).option('--url <url>').option('--timeout <duration>').action((src, o) => run(async () => { let html = o.url ? `<iframe src="${o.url}" style="border:0;width:100vw;height:100vh"></iframe>` : await htmlSource(src, o); if (o.url) await assertUrlAllowed(o.url, o.allowRemote); const res = await promptWindow(withBridge(html, o.csp ?? (o.allowRemoteResources ? undefined : DEFAULT_CSP)), { ...options(o), timeout: parseDuration(o.timeout) }); ok({ result: res ?? null }); }));
addOpts(addHtml(program.command('open'))).option('--url <url>').option('--watch').action((src, o) => run(async () => { let html = o.url ? `<iframe src="${o.url}" style="border:0;width:100vw;height:100vh"></iframe>` : await htmlSource(src, o); let security:any={}; if (o.url) security = await assertUrlAllowed(o.url, o.allowRemote); html = withBridge(html, o.csp ?? (o.allowRemoteResources || o.url ? undefined : DEFAULT_CSP)); const res = await request('open', { html, name: o.name, replace: o.replace, options: options(o), source: o.url ? { kind:'url', url:o.url } : { kind:'html', path: src }, bridge: !o.url || security.trusted || o.allowBridge, security }); ok(res); }));
