...
function addWindow(c: Command) { return c.requiredOption('-w, --window <ref>', 'Window id or window name returned by/opened with `glimpse open`.'); }
function addUrlPolicy(c: Command) { return c.option('--allow-remote', 'Allow non-loopback remote URLs. Loopback URLs are allowed by default.'); }
function addHtmlPolicy(c: Command) { return c.option('--allow-remote-resources', 'Do not apply the default restrictive CSP to inline/file HTML.').option('--csp <policy>', 'Custom Content-Security-Policy for inline/file HTML.'); }
function addOpenPolicy(c: Command) { return addHtmlPolicy(addUrlPolicy(c).option('--allow-bridge', 'Inject the Glimpse page bridge into remote URL content.')); }
function addPromptPolicy(c: Command) { return addHtmlPolicy(addUrlPolicy(c).option('--allow-bridge', 'Required for remote URL prompts so the page can return a result.')); }
function addHtml(c: Command) { return addHtmlPolicy(c.argument('[html-source]', 'HTML file path, `-` for stdin, or omit when using --html.').option('--html <literal>', 'Inline HTML literal.')); }
...
skills.command('view')
  ...
  .action((name?: string) => run(async () => {
    ...
    for (const skill of names) {
      if (!skillNames.includes(skill as any)) throw new Error(`Invalid skill ${skill}. Expected one of: ${skillNames.join(', ')}`);
...
skills.command('copy')
  ...
  .action((targetDir, o) => run(async () => {
    const target = resolve(String(targetDir));
    mkdirSync(target, { recursive: true });
    for (const skill of skillNames) cpSync(join(skillsDir, skill), join(target, skill), { recursive: true, force: Boolean(o.force), errorOnExist: !o.force });
...
Examples:
  ...
  $ glimpse open --url http://localhost:3000`).action((src, o) => run(async () => { if (o.watch && (!src || src === '-' || o.html != null || o.url)) throw new Error('usage: --watch requires a file-based html-source'); let html = o.url ? iframeForUrl(o.url) : await htmlSource(src, o); let security:any={}; if (o.url) security = await assertUrlAllowed(o.url, o.allowRemote); html = withBridge(html, o.csp ?? (o.allowRemoteResources || o.url ? undefined : DEFAULT_CSP)); const watchPath = o.watch ? resolve(String(src)) : undefined; const res = await request('open', { html, name: o.name, replace: o.replace, options: options(o), source: o.url ? { kind:'url', url:o.url } : { kind:'html', path: src, watch: Boolean(o.watch) }, bridge: !o.url || security.trusted || o.allowBridge, security, watchPath }); ok(res); }));
addHtml(addWindow(program.command('set-html').description('Replace the HTML content of an existing window.'))).addHelpText('after', `
Example:
  $ glimpse set-html -w demo --html '<h1>Updated</h1>'`).action((src, o) => run(async () => ok(await request('set-html', { window: o.window, html: withBridge(await htmlSource(src,o), o.csp ?? (o.allowRemoteResources ? undefined : DEFAULT_CSP)) }))));
...
Example:
  $ glimpse navigate -w demo --url http://localhost:3000`).action(o => run(async () => { await assertUrlAllowed(o.url, o.allowRemote); ok(await request('navigate', { window:o.window, url:o.url })); }));
...
Example:
  $ glimpse send -w demo --type app.update --data '{"status":"working"}'`).action(o => run(async () => { const set = [o.data!=null,o.dataFile!=null,o.text!=null].filter(Boolean).length; if (set !== 1) throw new Error('usage: choose exactly one of --data, --data-file, --text'); const data = o.text ?? (o.dataFile ? await readDataFile(o.dataFile) : parseJson(o.data,'data JSON')); ok(await request('send', { window:o.window, type:o.type, data })); }));
...
Example:
  $ glimpse eval -w demo 'document.title'`).action((js,o) => run(async () => ok(await request('eval', { window:o.window, js }))));
for (const name of ['read','wait','events','peek'] as const) {
  ...
  addWindow(program.command(name).description(description))
    ...
    .action(o => run(async () => ok(await request(name, { window:o.window, type:o.type, timeout:parseDuration(o.timeout) }))));
}
...
Examples:
  ...
  $ glimpse close --all`).action(o => run(async () => { if (!o.all && !o.window) throw new Error('usage: close requires -w or --all'); ok(await request('close', { window:o.window, all:o.all, force:o.force })); }));
