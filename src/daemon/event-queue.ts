...
export class EventQueue {
  ...
  constructor(private max = 1000) {
    this.max = Math.min(Math.max(max, 1), 20_000);
  }
  push(type: string, data?: unknown) {
    if (type.startsWith("window.") || type.startsWith("glimpse.") || type.startsWith("html."))
      type = "glimpse.error";
    this.events.push({ id: ++this.seq, type, data, timestamp: new Date().toISOString() });
    if (this.events.length > this.max) this.events.shift();
  }
  system(type: string, data?: unknown) {
    this.events.push({ id: ++this.seq, type, data, timestamp: new Date().toISOString() });
    if (this.events.length > this.max) this.events.shift();
  }
  peek(type?: string) {
  ...
  read(type?: string): WindowEvent | null {
