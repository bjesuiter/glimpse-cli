...
export async function dispatch(method: string, p: any = {}) {
  switch (method) {
    ...
    case "open": {
      ...
      return { windowId: w.id, name: w.name };
    }
    case "set-html":
    ...
    case "eval":
      ...
      return { ok: true };
    case "read":
      return { event: registry.resolve(p.window)?.queue.read(p.type) ?? null };
    case "peek":
    case "events":
      return { events: registry.resolve(p.window)?.queue.peek(p.type) ?? [] };
    case "wait":
